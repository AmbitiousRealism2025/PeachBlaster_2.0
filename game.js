...
function breakPeach(h){ const i=peaches.indexOf(h); if(i===-1) return; const s=h.userData.sizeCategory, p=h.position.clone(), o=h.userData.velocity; removeObject(h); peaches.splice(i,1); let n, l=0, c=0; if(s==='large'){ n='medium'; l=2; c=20; } else if(s==='medium'){ n='small'; l=3; c=50; } else { c=100; } updateScore(c); for(let k=0; k<l; k++){ const a=(Math.PI*2*k/l)+THREE.MathUtils.randFloatSpread(0.5); const b=SLICE_SPEED_MULTIPLIER+Math.random()*0.5; const v=new THREE.Vector3(o.x*0.5+Math.cos(a)*(PEACH_MAX_SPEED*b*0.5), o.y*0.5+Math.sin(a)*(PEACH_MAX_SPEED*b*0.5), 0); const S=createPeach(n, p.clone(), v); if(n==='medium'||n==='small'){ peaches.push(S); if(scene) scene.add(S); } } }
function createBullet(p,d,i){ const r=i?0.5:0.6, c=i?0x00ff00:0xffff00; const g=new THREE.SphereGeometry(r,8,8); const m=new THREE.MeshBasicMaterial({color:c}); const b=new THREE.Mesh(g,m); b.position.copy(p); b.userData={velocity:d.clone().multiplyScalar(BULLET_SPEED),radius:r,isPlayerBullet:i}; return b; }
...
function createSinglePlanet() { console.log("DEBUG: Creating single distant planet with gradient and rings..."); const planetRadius = 10; const planetColorLight = "#87CEEB"; const planetColorDark = "#4682B4"; const canvasSize = 128; const canvas = document.createElement('canvas'); canvas.width = canvasSize; canvas.height = canvasSize; const context = canvas.getContext('2d'); const gradient = context.createLinearGradient(0, 0, 0, canvasSize); gradient.addColorStop(0, planetColorLight); gradient.addColorStop(1, planetColorDark); context.fillStyle = gradient; context.fillRect(0, 0, canvasSize, canvasSize); const planetTexture = new THREE.CanvasTexture(canvas); planetTexture.needsUpdate = true; const planetGeometry = new THREE.SphereGeometry(planetRadius, 32, 24); const planetMaterial = new THREE.MeshStandardMaterial({ map: planetTexture, roughness: 0.8, metalness: 0.1 }); const planetMesh = new THREE.Mesh(planetGeometry, planetMaterial); planetMesh.name = "PlanetBody"; planetMesh.rotation.z = -Math.PI / 16; const ringDefs = [ { inner: planetRadius * 1.2, outer: planetRadius * 1.6, color: 0xC1B5A9 }, { inner: planetRadius * 1.6, outer: planetRadius * 1.68, color: 0x181818 }, { inner: planetRadius * 1.68, outer: planetRadius * 2.2, color: 0xAD9F93 }, { inner: planetRadius * 2.2, outer: planetRadius * 2.4, color: 0x948B80, opacity: 0.8 }, ]; const planetGroup = new THREE.Group(); planetGroup.add(planetMesh); const ringTiltY = Math.PI / 7; ringDefs.forEach((def, index) => { const ringGeometry = new THREE.RingGeometry(def.inner, def.outer, 64); const ringMaterial = new THREE.MeshStandardMaterial({ color: def.color, roughness: 0.85, metalness: 0.1, side: THREE.DoubleSide, transparent: def.opacity < 1.0, opacity: def.opacity !== undefined ? def.opacity : 1.0 }); const ringMesh = new THREE.Mesh(ringGeometry, ringMaterial); ringMesh.rotation.x = Math.PI / 2; ringMesh.rotation.y = ringTiltY; ringMesh.name = `PlanetRing_${index}`; planetGroup.add(ringMesh); }); const posX = WORLD_WIDTH * 0.65; const posY = -WORLD_HEIGHT * 0.5; const posZ = -230; planetGroup.position.set(posX, posY, posZ); planetGroup.rotation.z = Math.PI / 10; planetGroup.rotation.x = -Math.PI / 24; planetGroup.name = "DistantPlanet"; scene.add(planetGroup); console.log(`DEBUG: Added DistantPlanet group at Z=${posZ}`); }
